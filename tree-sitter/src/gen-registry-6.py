...
def generate_entt_registration(config: CSettings, namespace="anson"):
    ...
    for ename, evals in found_enums.items():
        output.append(f"{indent}entt::meta_factory<{namespace}::{ename}>().type(\"{ename}\"_hs)")
        for v in evals:
    ...
    for cname, info in found_classes.items():
        line = f"{indent}entt::meta_factory<{namespace}::{cname}>().type(\"{cname}\"_hs)"
        for params in info["ctors"]:
            line += f".ctor{params}()"
        if info["base"]:
            line += f".base<{namespace}::{info['base']}>()"
        for fname in info["fields"]:
            line += f".data<&{namespace}::{cname}::{fname}>(\"{fname}\"_hs, \"{fname}\")"
        output.append(line + ";\n")
