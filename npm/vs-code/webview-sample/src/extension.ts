...
export function activate(context: vscode.ExtensionContext) {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand('catCoding.startServer', () => {
			const port = '8888';
...
class CatCodingPanel {
	...
	private _loadOnline(page: string) {
		const webview = this._panel.webview;
		this.page = page;
	 ...
	 * @returns 
	 */
	private _getAnclientPage(page: string): string {
		return `<!DOCTYPE html>
		<html lang="en">
		<head>
			<meta charset="UTF-8">
	...
	private _getHtmlForWebview(catGifPath: string) {
		const webview = this._panel.webview;
