	...
	void generate () {
		...
		if (modules.contains(ProjectType.ANDROID)) {
			if (!GdxSetup.isSdkUpToDate(sdkLocation)) {
				// odys-z File sdkLocationFile = new File(sdkLocation);
		...
		new Thread() {
			public void run () {
				...
				new GdxSetup().build(builder, destination, name, pack, clazz, languageEnum, sdkLocation,
						// odys-z 
				ui.settings.gradleUrl.getText(),
				new CharCallback() {
					@Override
