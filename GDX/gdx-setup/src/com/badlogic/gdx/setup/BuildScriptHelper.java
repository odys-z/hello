...
public class BuildScriptHelper {
	...
	public static void addAllProjects(BufferedWriter wr,
			boolean... mirrorGFW // odys-z
			) throws IOException {
		...
		write(wr, DependencyBank.google);
		// odys-z
		if (mirrorGFW != null && mirrorGFW.length > 0 && mirrorGFW[0]) {
			write(wr, "maven { url \"https://maven.aliyun.com/repository/google\" }");
			write(wr, "maven { url \"https://maven.aliyun.com/repository/jcenter\" }");
		}
		write(wr, "maven { url \"" + DependencyBank.libGDXSnapshotsUrl + "\" }");
