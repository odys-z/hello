...
class Solution:
    ...
    def critConn(self, uix, pix):
        ...
        # visit u
        # verts[uix] == None
        u = {'ix': uix, 'low': self.time, 'disc': self.time, 'pix': pix, 'edges': self.edges[uix]}
        self.verts[u['ix']] = u
        self.time += 1
...
#                 self.verts[uix] = u
                self.verts[uix]['low'] = min(u['low'], v['low'])
...
#                 self.verts[u.ix] = u
                self.verts[u['ix']]['low'] = min(u['low'], v['disc'])
        return u
