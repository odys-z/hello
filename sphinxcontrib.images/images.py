...
class image_node(nodes.image, nodes.General, nodes.Element):
    def visit_image_node_html(self, writer):
        # print("===================== image_node.visit_image_node_html, super, sphinx_backend", super().visit_image_node_html, self.app.sphinxcontrib_images_backend)
        self._app.sphinxcontrib_images_backend.visit_image_node_html(writer, self)
...
class ImageDirective(Directive):
    ...
    option_spec = {
        ...
        'uri-preview': directives.unchanged,
    ...
    def update_preview(self, env, img, has_preview):
        def generate_thumbnail(env, img_path):
            from PIL import Image

            """Generate a thumbnail and return its path."""
            print("""Generate a thumbnail and return its path.""", img['uri'])
            # env = self.state.document.settings.env
            src_dir = env.srcdir
            build_dir = env.app.outdir
            rel_path = img_path.lstrip(os.sep)
            full_path = os.path.join(src_dir, rel_path)

            # Create thumbnail directory
            thumb_dir = os.path.join(build_dir, '_images', 'thumbnails')
            os.makedirs(thumb_dir, exist_ok=True)

            # Generate thumbnail
            thumb_name = f"thumb_{os.path.basename(img_path)}"
            thumb_path = os.path.join(thumb_dir, thumb_name)

            print("Generating thumbnail for:", full_path, "->", thumb_path)

            with Image.open(full_path) as img:
                img.thumbnail((200, 200))  # Set max thumbnail size
                img.save(thumb_path)
            return thumb_path

        print("====================== update preview, has_preview", has_preview, img['uri'])
        # prv = img['uri-preview'] if img['uri-preview'] else img['uri']
        if has_preview:
            prv = get_thumbname(img['uri'])
            if env.images.__contains__(prv):
                img['uri-preview'] = prv
            else:
                img['uri-preview'] = generate_thumbnail(env, img['uri'])
                env.images.add_file('', prv)
    ...
    def run(self):
        ...
        preview = self.options.get('uri-preview')
        print("==================== run().self.options: preview ", preview)
        ...
        if self.is_remote(self.arguments[0]):
            ...
            if download:
                ...
                img['remote_uri'] = self.arguments[0]
                self.update_preview(env, img, preview)
        else:
            img['uri'] = self.arguments[0]
            self.update_preview(env, img, preview)
            img['remote'] = False
        ...
        print("==================== run() preview ", preview)
        img['uri-preview'] = preview
    ...
    def backend_methods(node, output_type):
        def backend_method(f):
            ...
            def inner_wrapper(writer, node):
                # print("++++++++++++++++++++++ inner_wrapper, group:", node['group'])
                # if node['group'] == 'case00-imgs-1':
                #     print("============ inner_wrapper, group=case00-imgs-1:", writer.__class__.__name__, f.__name__)
                # img_uri = node['uri']
                # print("++++++++++++++++++++++ visit_" + writer.__class__.__name__ + '_' + output_type,
                #       hasattr(writer, 'visit_' + writer.__class__.__name__ + '_' + output_type),
                #       hasattr(writer, 'visit_image'))
                # if hasattr(writer, 'visit_image'):
                #     node['uri'] = 'unknown.png'
                # res = f(writer, node)
                # node['uri'] = img_uri
                # return res
        ...
        # print('================ visit_' + node.__name__ + '_fallback', 
        #                        getattr(backend, 'visit_' + node.__name__ + '_fallback')),
        # print('================ visit' + signature, getattr(backend, 'visit' + signature, 
        #                        getattr(backend, 'visit_' + node.__name__ + '_fallback'))),
